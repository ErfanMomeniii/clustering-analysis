...
def test_build_scaler_hybrid_returns_column_transformer():
    s = build_scaler("hybrid", v_features=[f"V{i}" for i in range(1, 29)], robust_features=["log_amount", "time_sin", "time_cos"])
...
def test_fit_and_describe_returns_per_strategy_summary():
    df = _processed_frame()
    desc = fit_and_describe(df, v_features=[f"V{i}" for i in range(1, 29)], robust_features=["log_amount", "time_sin", "time_cos"])
